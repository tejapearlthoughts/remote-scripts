...
class SonarQubeReportSlack:
    ...
    def generate_summary(self):
        url = self.sonar_url + "/api/issues/search?severities=BLOCKER,CRITICAL"
        res = requests.get(url, auth=(self.sonar_username, self.sonar_password)).json()
        print(res)
        count = str(res["paging"]["total"])
        return count,"SAST %s: %s Blocker and Critical Security Issues Identified in the Repository"%(self.component,count)
